...
distri_0s = {}
def init_model():
    with open("../model/retention0s.csv", "r") as f:
        lines = f.readlines()
        for line in lines:
            tokens = line.split(',')
            tmin, tmax, distr = int(tokens[0]), int(tokens[1]), list(map(int, tokens[2:]))
            distri_0s[(tmin, tmax)] = distr
    # print(distri_0s)
...
def level_inference(BER):
    ...
    for tmin in range(0, 60):
        tmax = tmin + 4
        RelaxDistr = distri_0s[(tmin, tmax)]
        if DEBUG:
...
def dump_to_json(level_alloc):
    ...
    write_to_json(bpc, f"../settings/{bits_per_cell}bpc_dala0s_{date}.json")
